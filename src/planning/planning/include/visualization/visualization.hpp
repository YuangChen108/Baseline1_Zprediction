...
class Visualization {
 ...
 public:
  std::shared_ptr<visualization::CameraVis> cam_vis_down_;
  std::shared_ptr<visualization::CameraVis> cam_vis_front_;

  Visualization(ros::NodeHandle& nh) : nh_(nh){
    cam_vis_down_ = std::make_shared<visualization::CameraVis>(nh, "down");
    cam_vis_front_ = std::make_shared<visualization::CameraVis>(nh, "front");
  }
  ...
  void visualize_odom(const Eigen::Vector3d& odomp, const Eigen::Vector3d& odomv,
                      const Eigen::Quaterniond& odomq, const TOPIC& topic) {
    auto got = publisher_map_.find(topic);
    if (got == publisher_map_.end()) {
    ...
    odom_msg.pose.pose.position.x = odomp.x();
    odom_msg.pose.pose.position.y = odomp.y();
    odom_msg.pose.pose.position.z = odomp.z();
    odom_msg.pose.pose.orientation.w = odomq.w();
    odom_msg.pose.pose.orientation.x = odomq.x();
    odom_msg.pose.pose.orientation.y = odomq.y();
    odom_msg.pose.pose.orientation.z = odomq.z();
    odom_msg.twist.twist.linear.x = odomv.x();
    odom_msg.twist.twist.linear.y = odomv.y();
    odom_msg.twist.twist.linear.z = odomv.z();

    publisher_map_[topic].publish(odom_msg);
  }
