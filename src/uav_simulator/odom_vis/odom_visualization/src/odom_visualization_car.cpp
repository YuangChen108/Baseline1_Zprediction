...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  pose(0) = msg->pose.pose.position.x;
  pose(1) = msg->pose.pose.position.y;
  pose(2) = msg->pose.pose.position.z; // ✅ 关键修改：读取真实高度，不再设为 0

  // 1.2 读取姿态 (直接读取，不要乱改顺序)
  // 原代码里有 q(0)=z, q(3)=-w 这种奇怪的交换，是因为之前的 mesh 轴向不对。
  // 现在用 CUBE (方块)，直接读原始数据即可。
  q(0) = msg->pose.pose.orientation.w;
  q(1) = msg->pose.pose.orientation.x;
  q(2) = msg->pose.pose.orientation.y;
  q(3) = msg->pose.pose.orientation.z;

  // 1.3 辅助变量更新 (用于保留原来的 Path/Velocity 功能)
  pose.rows(3, 5) = R_to_ypr(quaternion_to_R(q));
  vel(0) = msg->twist.twist.linear.x;
  vel(1) = msg->twist.twist.linear.y;
  vel(2) = msg->twist.twist.linear.z;

  // 1.4 准备 Eigen 变量 (方便计算坐标)
  Eigen::Vector3d t_eigen(pose(0), pose(1), pose(2));
  Eigen::Quaterniond q_eigen(q(0), q(1), q(2), q(3));

  // ==================== 2. 更新 FOV 可视化 ====================
  // 顺便把这里的输入也修正为 t_eigen 和 q_eigen，保证视野跟随船晃动
  pub_fov_visual(t_eigen, q_eigen);

  // ==================== 3. 核心：画单层方形板 (替代 Mesh) ====================
  
  // 复用原来的 meshROS 变量，但彻底改变它的属性
  meshROS.header.frame_id = _frame_id;
  meshROS.header.stamp = msg->header.stamp;
  meshROS.ns = "landing_platform"; // 修改命名空间
  meshROS.id = 0;
  meshROS.type = visualization_msgs::Marker::CUBE; // ✅ 改为方块
  meshROS.action = visualization_msgs::Marker::ADD;

  // --- 计算位置偏移 ---
  // 板子厚 0.2m。如果不加偏移，板子中心在水面 (Z=0)，会有一半沉在水下。
  // 加 0.1m 偏移，让板子底面刚好贴着水面。
  Eigen::Vector3d offset(0, 0, 0.1); 
  Eigen::Vector3d final_pos = t_eigen + q_eigen * offset;

  meshROS.pose.position.x = final_pos.x();
  meshROS.pose.position.y = final_pos.y();
  meshROS.pose.position.z = final_pos.z();

  meshROS.pose.orientation.w = q_eigen.w();
  meshROS.pose.orientation.x = q_eigen.x();
  meshROS.pose.orientation.y = q_eigen.y();
  meshROS.pose.orientation.z = q_eigen.z();

  // --- 设置尺寸 (1.5m x 1.5m x 0.2m) ---
  meshROS.scale.x = 1.5;
  meshROS.scale.y = 1.5;
  meshROS.scale.z = 0.2;
  ...
  meshROS.color.g = 0.5;
  meshROS.color.b = 0.0;
  meshROS.color.a = 0.9; // 略微透明

  meshROS.mesh_use_embedded_materials = false; // 不需要嵌入材质
  // meshROS.mesh_resource = ...; // 不需要加载资源文件了
